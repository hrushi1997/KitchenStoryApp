...
export class KitchenitemsComponent implements OnInit {
  ...
  set searchTerm(value: string) {
    ...
    this.searchedItems = this.searchTerm
      ? this.searchItem(this.searchTerm)
      : this.kitchenitems;
  }
  searchItem(searchby: string): Mykitchen[] {
    searchby = searchby.toLocaleLowerCase();
    return this.kitchenitems.filter(
      (kitchen: Mykitchen) =>
        kitchen.vegetablename.toLocaleLowerCase().indexOf(searchby) !== -1
    );
  }
  ...
  fun(): void {
    ...
    this.route.navigate(['/kitchendetail']);
  }
  fun2(): void {
    //window.alert('Please Login to purchase');
    //this.route.navigate(['/login']);
